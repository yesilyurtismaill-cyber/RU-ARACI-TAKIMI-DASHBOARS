...
(() => {
  ...
  function sellerTable(rows) {
    ...
    return `<div class="seller-overview">
      ...
      <div class="scroll"><table class="seller-performance-table"><thead><tr><th>#</th><th>Satıcı</th><th>Gelen Lead</th><th>Teklif Adedi</th><th>Toplam Teklif Tutarı</th><th>Satış Adedi</th><th>Satış Tutarı</th><th>Teklif → Satış</th><th>Lead → Satış</th><th>Ort. Satış</th></tr></thead><tbody>${rows.map((r,i)=>`<tr><td><span class="rank ${i===0?"gold":""}">${i+1}</span></td><td><b>${esc(r.sellerLabel||r.seller)}</b></td><td><span class="seller-count lead">${number(r.lead)}</span></td><td><span class="seller-count quote">${number(r.quote)}</span></td><td class="seller-quote-amount">${money(r.quoteAmount)}</td><td><span class="seller-count won">${number(r.won)}</span></td><td class="seller-sales">${money(r.sales)}</td><td><div class="seller-rate ${rateTone(r.quoteToWon)}"><b>${pct(r.quoteToWon)}</b><i><em style="width:${Math.min(100,Math.max(0,Number(r.quoteToWon||0)*100))}%"></em></i></div></td><td><div class="seller-rate ${rateTone(r.conversion)}"><b>${pct(r.conversion)}</b><i><em style="width:${Math.min(100,Math.max(0,Number(r.conversion||0)*100))}%"></em></i></div></td><td>${money(r.avgSale)}</td></tr>`).join("")}</tbody></table></div>
    </div>`;
  }
