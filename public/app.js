...
(() => {
  ...
  function addStyles() {
    ...
    style.textContent = `
      ...
      *{box-sizing:border-box}html,body{margin:0;min-height:100%;font-family:Arial,Helvetica,sans-serif;background:var(--bg);color:var(--ink)}button,input,select{font:inherit}button{cursor:pointer}
      ...
      .seller-totals.general-sales-totals{grid-template-columns:repeat(4,1fr)}@media(max-width:1100px){.seller-totals.general-sales-totals{grid-template-columns:repeat(4,1fr)}}@media(max-width:650px){.seller-totals.general-sales-totals{grid-template-columns:repeat(2,1fr)}}
    `;
  ...
  function sellerTable(rows) {
    ...
    const salesDetail = row => {
      ...
      const detailRows = details.map((sale, index) => `<tr><td>${index + 1}</td><td><b>${esc(sale.id || "-")}</b></td><td>${esc(sale.patientName || "İsimsiz vaka")}</td><td>${esc(displayDate(sale.dealWonDate))}</td><td class="amount">${money(sale.amountUsd)}</td><td>${esc(sale.coordinator || "-")}</td><td>${esc(sale.sourceCategory || "-")}</td><td>${esc(sale.department || "-")}</td><td>${esc(sale.doctor || "-")}</td></tr>`).join("");
      return `<tr class="seller-sales-detail"><td colspan="10"><div class="seller-sales-detail-wrap"><div class="seller-sales-detail-title"><span>${esc(row.sellerLabel || row.seller)} · Facilitator kaynaklı satışlar</span><small>${number(details.length)} vaka · ${money(details.reduce((sum, sale) => sum + Number(sale.amountUsd || 0), 0))}</small></div><div class="seller-sales-detail-table scroll"><table><thead><tr><th>#</th><th>Bitrix ID</th><th>Hasta Adı</th><th>Satış Tarihi</th><th>Satış Tutarı</th><th>Aracı</th><th>Source Kategorisi</th><th>Bölüm</th><th>Doktor</th></tr></thead><tbody>${detailRows || `<tr><td colspan="9" class="empty">Bu satıcı için facilitator kaynaklı satış bulunamadı.</td></tr>`}</tbody></table></div></div></td></tr>`;
    };
    ...
    return `<div class="seller-overview">
      <div class="seller-totals"><div><span>Toplam Gelen Lead</span><b>${number(totals.lead)}</b></div><div><span>Teklif Adedi</span><b>${number(totals.quote)}</b></div><div><span>Toplam Teklif Tutarı</span><b>${money(totals.quoteAmount)}</b></div><div><span>Facilitator Satış Adedi</span><b>${number(totals.won)}</b></div><div><span>Facilitator Satış Tutarı</span><b>${money(totals.sales)}</b></div><div><span>Teklif → Facilitator Satış</span><b>${pct(totals.quote?totals.won/totals.quote:0)}</b></div><div><span>Lead → Facilitator Satış</span><b>${pct(totals.lead?totals.won/totals.lead:0)}</b></div></div>
  ...
  function generalSalesTable(rows) {
    if (!rows?.length) return `<div class="empty">Bu filtrelerde genel satış verisi bulunamadı.</div>`;
    const totals = rows.reduce((acc, row) => ({ won: acc.won + Number(row.won || 0), sales: acc.sales + Number(row.sales || 0) }), { won: 0, sales: 0 });
    const detailRow = row => {
      const details = Array.isArray(row.salesDetails) ? row.salesDetails : [];
      const body = details.map((sale, index) => `<tr><td>${index + 1}</td><td><b>${esc(sale.id || "-")}</b></td><td>${esc(sale.patientName || "İsimsiz vaka")}</td><td>${esc(displayDate(sale.dealWonDate))}</td><td class="amount">${money(sale.amountUsd)}</td><td>${esc(sale.coordinator || "-")}</td><td>${esc(sale.sourceCategory || "-")}</td><td>${esc(sale.department || "-")}</td><td>${esc(sale.doctor || "-")}</td></tr>`).join("");
      return `<tr class="seller-sales-detail"><td colspan="6"><div class="seller-sales-detail-wrap"><div class="seller-sales-detail-title"><span>${esc(row.sellerLabel || row.seller)} · Tüm satışlar</span><small>${number(details.length)} vaka · ${money(details.reduce((sum, sale) => sum + Number(sale.amountUsd || 0), 0))}</small></div><div class="seller-sales-detail-table scroll"><table><thead><tr><th>#</th><th>Bitrix ID</th><th>Hasta Adı</th><th>Satış Tarihi</th><th>Satış Tutarı</th><th>Aracı</th><th>Source Kategorisi</th><th>Bölüm</th><th>Doktor</th></tr></thead><tbody>${body || `<tr><td colspan="9" class="empty">Bu satıcı için satış detayı bulunamadı.</td></tr>`}</tbody></table></div></div></td></tr>`;
    };
    const body = rows.map((row, index) => {
      const isOpen = state.expandedGeneralSellerSales === row.seller;
      const hasSales = Number(row.won || 0) > 0;
      const mainRow = `<tr><td><span class="rank ${index === 0 ? "gold" : ""}">${index + 1}</span></td><td><b>${esc(row.sellerLabel || row.seller)}</b></td><td><button class="seller-count won seller-sales-button ${isOpen ? "active" : ""}" data-general-seller-sales="${esc(row.seller)}" ${hasSales ? "" : "disabled"} title="${hasSales ? "Tüm satılan vakaları göster" : "Satış bulunmuyor"}" aria-expanded="${isOpen}">${number(row.won)} ${hasSales ? `<span aria-hidden="true">${isOpen ? "▲" : "▼"}</span>` : ""}</button></td><td class="seller-sales">${money(row.sales)}</td><td>${money(row.avgSale)}</td><td>${pct(row.salesShare)}</td></tr>`;
      return mainRow + (isOpen ? detailRow(row) : "");
    }).join("");
    return `<div class="seller-overview"><div class="seller-totals general-sales-totals"><div><span>Toplam Satış Adedi</span><b>${number(totals.won)}</b></div><div><span>Toplam Satış Tutarı</span><b>${money(totals.sales)}</b></div><div><span>Ortalama Satış</span><b>${money(totals.won ? totals.sales / totals.won : 0)}</b></div><div><span>Kapsam</span><b>Tüm Source Kategorileri</b></div></div><div class="scroll"><table class="seller-performance-table"><thead><tr><th>#</th><th>Satıcı</th><th>Satış Adedi</th><th>Satış Tutarı</th><th>Ort. Satış</th><th>Satış Payı</th></tr></thead><tbody>${body}</tbody></table></div></div>`;
  }
  ...
  function renderDashboard() {
    ...
    $("#appContent").innerHTML = `
      ...
      ${salesFlow(k)}
      <section class="panel dashboard-panel accent-purple"><div class="panel-head"><h3>Satıcı Teklif ve Aracı Kaynaklı Satış Performansı</h3><span class="muted">Satışlarda yalnızca Source Kategorisi: FACILITATORS</span></div>${sellerTable(d.sellerPerformance)}</section>
      ...
      <section class="panel dashboard-panel accent-green"><div class="panel-head"><h3>Genel Satış</h3><span class="muted">Source kategorisinden bağımsız · tüm satışlar</span></div>${generalSalesTable(d.generalSellerSales)}</section>
  ...
  function bindDashboardFilters() {
    document.querySelectorAll("[data-period]").forEach(button => button.addEventListener("click", () => {
      ...
      state.expandedGeneralSellerSales = "";
    ...
    document.querySelectorAll("[data-general-seller-sales]").forEach(button => button.addEventListener("click", () => {
      const seller = button.dataset.generalSellerSales || "";
      state.expandedGeneralSellerSales = state.expandedGeneralSellerSales === seller ? "" : seller;
      renderDashboard();
    }));
